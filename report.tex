...
the compilation phases explicit and easy to demonstrate.
...
\section{Changes Asked for During the Presentation}

During the presentation, an additional lexer-level validation was requested for
malformed numeric tokens that look like identifiers. The lexer was updated to
reject these cases immediately instead of allowing them to continue into later
stages.

Here is a program along with its output after the changes.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/lex_error_code.png}
    \caption{A program with a lexer error where an identifier starts with a number.}
    \label{fig:my_image}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{A screenshot of lexer error output displayed in the terminal.}
    \label{fig:my_image}
\end{figure}
...
\begin{lstlisting}
./bin/cforge samples/slice9_run.c -o program.vmo
\end{lstlisting}

Runtime execution:

\begin{lstlisting}
./bin/crun program.vmo
\end{lstlisting}
...
\begin{lstlisting}
make run FILE=samples/slice9_run.c OBJ=program.vmo
make phases FILE=samples/slice8_run.c OBJ=program.vmo
\end{lstlisting}
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Implementation in Go instead of C/C++}
...
These additions strengthen the project in terms of completeness, maintenance, and modularity.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/phases.png}
    \caption{A program compiled with our compiler showing all of the compilation phases.}
    \label{fig:my_image}
\end{figure}
